...
//            });
    ...
    public function scheduleFetchData()
    {
        try {
            $url = 'https://imunizacao-es.saude.gov.br/desc-imunizacao/_search?q=estabelecimento_uf:SE';
            ...
            if ($response->successful()) {
                ...
                Log::error('Falha ao buscar dados da API: ' . $response->status());
            }
        } catch (\Exception $e) {
            Log::error('Ocorreu uma exceção durante a busca de dados: ' . $e->getMessage());
        }
    }
}
